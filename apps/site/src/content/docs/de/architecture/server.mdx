...
Die Middleware liest das JWT aus dem Header `Authorization: Bearer <token>` (Cookie-Fallback bei Lesezugriffen), prüft es und hängt den Benutzerkontext an die Anfrage. Geschützte Routen nutzen `RequireAccess` für Rollen.

- **Zweck-Tokens** (`email_verify`, `password_reset`) teilen den Signaturschlüssel, dürfen aber **nicht** als Access-Token auf `Protected`-Routen dienen.
- **Mutationen** (POST/PUT/PATCH/DELETE auf authentifizierten Routen) erfordern einen Bearer-Header via `RequireBearerForMutations` — reine Cookie-POSTs cross-site werden abgelehnt (CSRF-Schutz).
- **Widerruf von Access-Tokens** (Logout / Passwortänderung) wird in der Datenbank persistiert und überlebt Neustarts sowie Multi-Instance-Deployments.
- **Raum-Präsenz**-Identitätslisten erfordern Authentifizierung; öffentliche Räume können `?countOnly=1` anbieten (nur Anzahl, keine Identitäten).
...
### Aufzeichnungen & LiveKit Egress

Queue-Handler **`process_recording`** und **`recording_delete`** sind am Worker registriert. LiveKit-Webhooks `egress_ended` stellen `process_recording` in die Queue (Download → Speichern → als fertig markieren → optional Webhook `recording.completed`).

🚧 **HTTP-Aufzeichnungsrouten** (Start/Stop/Liste über die API) bleiben im Produktion-Routentable `// TODO oncoming feature` — betreiben Sie öffentliche Recording-REST-Endpunkte erst, wenn sie freigeschaltet sind. YAML-Schlüssel unter `recording:` konfigurieren den Speicher der Pipeline.
...
- See the [Webhooks API](/de/docs/api/api-reference) for endpoint details.
...
- [Backend Code Structure](/de/docs/backend/structure) - directory map and coding standards
- [API Handlers](/de/docs/backend/api-handlers) - routing and request lifecycle
- [Database and Models](/de/docs/backend/database) - GORM models and repository pattern
- [Authentication Flow](/de/docs/backend/authentication) - JWT, OAuth, and passkey internals
