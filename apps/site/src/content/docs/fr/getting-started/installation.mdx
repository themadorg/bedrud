...
This guide covers installing the **Bedrud server** (API, web UI, WebRTC media server) on a Linux server. For desktop and mobile client apps, see [Client Installation](/fr/docs/getting-started/clients).
...
| Requirement | Details |
|-------------|---------|
| **Operating system** | Debian 12+, Ubuntu 22.04+, Arch Linux (rolling), Fedora 39+, Alpine 3.21+ |
...
> **No domain?** Bedrud works with an IP address using self-signed TLS. Browsers will show a certificate warning but video and audio work. For trusted certificates on internal networks, see the [Internal TLS Guide](/fr/docs/guides/internal-tls).
...
<Callout type="tip">
    Want to customize the install? Use flags like `--version 0.12.0`, `--install-dir /opt/bedrud`, or `--skip-shell`. Full reference: [CLI Installer](/fr/docs/getting-started/cli-installer).
</Callout>
...
<Tabs>
  <TabsList>
    ...
    <TabsTrigger value="alpine-apk">Alpine (apk)</TabsTrigger>
  </TabsList>
  ...
  <TabsContent value="fedora-rhel-dnf">
    ...
    ```bash
    sudo curl -fsSL https://bedrud.org/key.gpg \
      -o /etc/pki/rpm-gpg/RPM-GPG-KEY-bedrud

    sudo curl -fsSL https://bedrud.org/dnf/bedrud.repo \
  ...
  <TabsContent value="alpine-apk">

    Download the `.apk` package from the [latest release](https://github.com/themadorg/bedrud/releases/latest):
...
<Callout type="tip">
    For desktop client installation (Linux, macOS, Windows) see [Client Installation](/fr/docs/getting-started/clients).
</Callout>
...
Full Docker documentation including volumes, reverse proxy, and TLS setup: [Docker Guide](/fr/docs/guides/docker).
...
| `--fresh` | `false` | Wipe existing install (config, data, certs) then reinstall. Without this, existing config files are left unchanged |
...
If using auto-TLS (ports 80/443) or a custom `httpPort`, allow those too:
...
Download the latest release and re-run install. Without `--fresh`, the installer refreshes the binary and services and **does not overwrite** existing `config.yaml` / `livekit.yaml`. For a binary-only upgrade prefer `bedrud update latest`. A clean wipe uses `--fresh`:

```bash
curl -fsSL -o bedrud.tar.xz https://github.com/themadorg/bedrud/releases/latest/download/bedrud_linux_amd64.tar.xz
...
```bash
docker pull ghcr.io/themadorg/bedrud:latest
# or: docker pull ghcr.io/themadorg/bedrud:latest-alpine
# or: docker pull ghcr.io/themadorg/bedrud:latest-distroless
...
Upgrade an existing server install **without** wiping config, secrets, certificates, or the database.

`bedrud update` and `bedrud upgrade` are the same command.

### Binary / release install

The shortest path downloads and applies the latest release in one step:

```bash
sudo bedrud update --check    # what it would install — changes nothing
sudo bedrud update latest     # download, verify, and apply it
```

To fetch the binary yourself first, apply it with its absolute path so an
older `bedrud` earlier in `PATH` cannot run the update instead:

```bash
# 1. Get the new binary (example: installer without re-running setup).
#    Run the installer as root so it lands in /usr/local/bin — without sudo
#    it installs to ~/.local/bin and step 2 would re-apply the old binary.
curl -fsSL https://bedrud.org/install.sh | sudo bash -s -- --no-setup

# 2. Apply binary + migrations + service restart
sudo /usr/local/bin/bedrud update --self
```

Or run the new release binary directly:

```bash
sudo ./bedrud update --self
```

Every form prints the installed version and the version it is moving to before
it stops anything, so you can cancel if the numbers are not what you expect.

### Package manager

```bash
# Debian / Ubuntu
sudo apt update && sudo apt install --only-upgrade bedrud
sudo bedrud update --skip-binary   # if postinst already restarted; still runs migrations
...
| Kept | Changed |
|------|---------|
| `/etc/bedrud/config.yaml` (secrets, domain, TLS paths) | Installed binary |
| Certificates | Init service unit files (refreshed) |
| SQLite/Postgres data | DB schema (AutoMigrate) |
| Logs under `/var/log/bedrud` | `/var/lib/bedrud/version` |

For a clean reinstall that regenerates secrets, use `sudo bedrud install --fresh` (destructive).

Full CLI flags: [CLI Reference — update / upgrade](/fr/docs/getting-started/cli-reference#update--upgrade).
...
| **Port 80 permission denied** | Non-root can't bind ports < 1024. Set `httpPort: "8080"` in config, or `sudo setcap 'cap_net_bind_service=+ep' $(which bedrud)` (re-run after each update) |
...
| **Self-signed cert warnings** | Expected. See [Internal TLS Guide](/fr/docs/guides/internal-tls) for private CA setup |
