...
3. **Автоподстановка** – `authFetch` оборачивает каждый API-вызов и добавляет заголовок `Authorization: Bearer` (обязателен для изменяющих запросов; сервер отклоняет cookie-only мутации)
...
End-user how-tos: [Using Bedrud](/ru/docs/using/joining-a-meeting) (start at joining, then stage, whiteboard, chat, and controls).
