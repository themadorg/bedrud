...
The Android app has its own repository, [bedrud-android](https://github.com/themadorg/bedrud-android), and is included here as a git submodule at `apps/android` so full-product builds (CI, releases) can still be produced from a single clone of this repo. For day-to-day Android development, work directly in bedrud-android — see its README for setup.
...
### Product surface (what users get)

| Area | Behavior |
|------|----------|
| Auth | Login / register (and passkeys where supported) |
| Dashboard | Room list; **recent rooms** after a join |
| Deep links | Pasted / opened room URLs resolve to the correct room |
| Meeting | Join, mute, leave; usable controls chrome |
| Stage / share | Stage overlay and screen-share path when the device allows |
| Telecom | Self-managed call connection; disconnect ends the meeting cleanly |
| RTL | `fa` / `ar` locales with Shabnam — chrome should not clip |

Shared product concepts (stage, chat, whiteboard on web) are documented under [Using Bedrud](/en/docs/using/joining-a-meeting). Feature parity with web may differ by release.
