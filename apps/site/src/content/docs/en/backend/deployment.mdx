...
Re-running `bedrud install` without `--fresh` does **not** overwrite `/etc/bedrud/config.yaml` or `livekit.yaml`. Use `bedrud update` to upgrade the binary, or `bedrud install --fresh` to wipe and regenerate config (destructive).
...
Replaces the binary with the running executable, runs versioned install migrations and DB AutoMigrate, refreshes init units, and restarts services. See [Updating](/en/docs/getting-started/installation#updating).
