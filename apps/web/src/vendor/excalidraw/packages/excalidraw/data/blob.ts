...
// Static: utils/export already pulls image.ts into the graph, so a dynamic
// import here is ineffective for code-splitting (Rolldown INEFFECTIVE_DYNAMIC_IMPORT).
import { decodePngMetadata } from "./image";
...
const parseFileContents = async (blob: Blob | File): Promise<string> => {
  ...
  if (blob.type === MIME_TYPES.png) {
    try {
      return await decodePngMetadata(blob);
    } catch (error: any) {
