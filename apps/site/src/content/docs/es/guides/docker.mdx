...
Se publican tres variantes de imagen en GitHub Container Registry:

| Tag | Base | Tamaño | Ideal para |
|-----|------|--------|------------|
| `:latest` | Debian Bookworm | ~50 MB | Predeterminado — compatible con la mayoría de sistemas |
| `:latest-alpine` | Alpine 3.21 | ~30 MB | Hosts Alpine, huella mínima |
| `:latest-distroless` | Distroless | ~25 MB | Superficie de ataque mínima, sin shell |

Todas las imágenes contienen el mismo binario estático — la imagen base solo afecta las utilidades en tiempo de ejecución (ca-certs, datos de zona horaria).
...
# Distroless (mínima, sin shell)
docker pull ghcr.io/bedrud-ir/bedrud:latest-distroless

# Versión específica
docker pull ghcr.io/bedrud-ir/bedrud:v1.0.0
docker pull ghcr.io/bedrud-ir/bedrud:v1.0.0-alpine
docker pull ghcr.io/bedrud-ir/bedrud:v1.0.0-distroless
```
...
services:
  bedrud:
    # Usa :latest-alpine o :latest-distroless para imágenes más pequeñas
