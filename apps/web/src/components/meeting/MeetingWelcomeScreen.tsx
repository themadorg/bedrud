    ...
    async function refresh() {
      try {
        const res = await fetch(
          `${API_URL}/api/room/${encodeURIComponent(roomId)}/presence?countOnly=1`,
          { credentials: 'include' },
        )
        if (cancelled) return
        if (res.status === 429) {
      ...
      <div className="meet-welcome-panel meet-prejoin-panel flex max-h-full w-full flex-col gap-5 overflow-y-auto meet-scroll">
        <div>
          ...
          {isPublic && presenceCount !== null && (
            <p className="m-0 mt-1 text-xs text-[var(--meet-fg-subtle)]">{welcomePresenceCountLabel(presenceCount)}</p>
          )}
        </div>
