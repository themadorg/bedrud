...
This guide covers installing the **Bedrud server** (API, web UI, WebRTC media server) on a Linux server. For desktop and mobile client apps, see [Client Installation](/de/docs/getting-started/clients).
...
> **No domain?** Bedrud works with an IP address using self-signed TLS. Browsers will show a certificate warning but video and audio work. For trusted certificates on internal networks, see the [Internal TLS Guide](/de/docs/guides/internal-tls).
...
<Callout type="tip">
    Want to customize the install? Use flags like `--version 0.12.0`, `--install-dir /opt/bedrud`, or `--skip-shell`. Full reference: [CLI Installer](/de/docs/getting-started/cli-installer).
</Callout>
...
<Callout type="tip">
    For desktop client installation (Linux, macOS, Windows) see [Client Installation](/de/docs/getting-started/clients).
</Callout>
...
Full Docker documentation including volumes, reverse proxy, and TLS setup: [Docker Guide](/de/docs/guides/docker).
...
Full CLI flags: [CLI Reference — update / upgrade](/de/docs/getting-started/cli-reference#update--upgrade).
...
| **Self-signed cert warnings** | Expected. See [Internal TLS Guide](/de/docs/guides/internal-tls) for private CA setup |
