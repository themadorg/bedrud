...
| [Appliance Mode](/zh/docs/guides/appliance) | Single-binary all-in-one setup |
...
See the [Package Installation guide](/zh/docs/guides/packages) for full instructions, including adding the apt GPG key and repository.
...
After installation, run the interactive installer to configure TLS, init services (systemd/OpenRC/SysV), and the database:
...
See the [Installation Guide](/zh/docs/getting-started/installation) for all installation scenarios.
...
When server TLS is enabled in Docker (via `SERVER_ENABLE_TLS=true` with mounted certs), the embedded LiveKit process automatically configures TURN/TLS using the server's certificate. For custom LiveKit configuration, mount a LiveKit YAML and set `LIVEKIT_CONFIG_PATH`.
...
See the [Docker Guide](/zh/docs/guides/docker) for full details including volumes, configuration, and Docker Compose.
...
The server exposes two endpoints for load balancer and orchestration probes:

| Endpoint | Returns | Use |
|----------|---------|-----|
| `GET /api/health` | `{"status":"healthy","time":<unix>}` | Liveness check |
| `GET /api/ready` | `{"status":"ready","time":<unix>}` | Readiness check |

Root-level redirects (`/health` → `/api/health`, `/ready` → `/api/ready`) exist for backward compatibility.
...
| Docker image (`ghcr.io/themadorg/bedrud`) | Multi-arch container image pushed to GHCR |
