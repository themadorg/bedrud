...
**See also:** [Quick Start](/en/docs/getting-started/quickstart) | [Installation](/en/docs/getting-started/installation) | [Deployment Guide](/en/docs/guides/deployment) | [Docker Guide](/en/docs/guides/docker) | [CLI Reference](/en/docs/getting-started/cli-reference)
...
# Experimental WebXDC mini-apps (requires server.domain — not IP-only)
# webxdc:
#   enabled: false                         # Env: WEBXDC_ENABLED
#   baseDomain: "wx.example.com"           # instances: webxdc-<id>.wx.example.com — Env: WEBXDC_BASE_DOMAIN
#   uploadPolicy: owner_mod                # owner_mod | any_member
#   storageDir: "./data/webxdc"
# DNS: *.{baseDomain} must point to the same IP as the main site; TLS must cover *.{baseDomain}
```

Users still turn on **Settings → Experimental → WebXDC mini-apps** in the web client. See [WebXDC mini-apps](/en/docs/using/webxdc).
...
| Field | Meaning |
|-------|---------|
| `backend` | `disk` (default), `s3`, or `inline` |
| `maxBytes` | Hard per-file limit (default 10 MB). Overridable at runtime via admin `chatUploadMaxBytes`. |
| `maxDimension` | Max image width/height in pixels (default 8192). Overridable via admin `chatUploadMaxDimension`. |
| `inlineMaxBytes` | Smaller images returned as data URIs |
| `diskDir` | Path for disk backend |
| `s3.*` | S3-compatible credentials and public base URL |

The meeting UI and `GET /api/auth/settings` expose the effective `chatUploadMaxBytes` and `chatUploadMaxDimension` so clients can pre-validate before upload. The upload API still enforces the same limits (`413` oversize, `400` over-dimension).
...
See [WebRTC Connectivity](/en/docs/architecture/webrtc-connectivity) and [TURN Server](/en/docs/architecture/turn-server).
