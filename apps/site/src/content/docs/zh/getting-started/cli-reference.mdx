...
| 标志     | 默认值       | 描述                                                     |
|----------|--------------|----------------------------------------------------------|
| `--role` | `superadmin` | 目标角色：`superadmin`、`admin`、`moderator`、`user`、`guest` |
...
| 标志     | 默认值       | 描述                                      |
|----------|--------------|-------------------------------------------|
| `--role` | `superadmin` | 要移除的角色：`superadmin`、`admin`、`moderator` |
...
| 全局标志 | 默认值                     | 描述                         |
|----------|----------------------------|------------------------------|
| `--config` | `/etc/bedrud/config.yaml` | Bedrud 配置文件路径          |
...
**SANs：** 从服务器配置中派生 — `domain`（如果已设置），以及 `localhost`、`127.0.0.1` 和 `::1`。

**实现：** `server/internal/utils/tls.go`（`RenewSelfSignedCert`）
...
**实现：** `server/internal/utils/tls.go`（`ValidateTLSCertPair`）
