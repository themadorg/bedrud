...
export const textWysiwyg = ({
  ...
  const updateWysiwygStyle = () => {
    ...
    const { textAlign: storedTextAlign, verticalAlign } = updatedTextElement;
    const rtl = isRTL(editable.value || updatedTextElement.originalText);
    const textAlign = rtl && storedTextAlign === "left" ? "right" : storedTextAlign;
  ...
  if (onChange) {
    ...
    editable.oninput = () => {
      ...
      if (isRTL(editable.value)) {
        const current = app.scene.getElement<ExcalidrawTextElement>(id);
        if (current?.textAlign === "left") {
          app.scene.mutateElement(current, { textAlign: "right" });
        }
      }
