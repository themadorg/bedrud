...
description: Conception et composants du serveur backend
order: 7
---
...
Le middleware extrait le JWT de l'en-tête `Authorization: Bearer <token>` (repli cookie en lecture), le valide et attache le contexte utilisateur à la requête. Les routes protégées utilisent `RequireAccess` pour les rôles.

- Les **jetons à usage** (`email_verify`, `password_reset`) partagent la clé de signature mais **ne peuvent pas** servir de jeton d'accès sur les routes `Protected`.
- Les **mutations** (POST/PUT/PATCH/DELETE authentifiées) exigent un en-tête Bearer via `RequireBearerForMutations` — les POST cookie seuls cross-site sont rejetés (défense CSRF).
- La **révocation des jetons d'accès** (déconnexion / changement de mot de passe) est persistée en base et survit aux redémarrages et déploiements multi-instances.
- Les listes d'**identités de présence** de salle exigent une authentification ; les salles publiques peuvent exposer `?countOnly=1` (compte seul, sans identités).
...
### Enregistrements et LiveKit Egress

Les handlers de file **`process_recording`** et **`recording_delete`** sont enregistrés sur le worker. Les webhooks LiveKit `egress_ended` enfilent `process_recording` (téléchargement → stockage → marquer terminé → webhook optionnel `recording.completed`).

🚧 Les **routes HTTP d'enregistrement** (start/stop/list via l'API) restent `// TODO oncoming feature` sur la table de routes de production — ne vous fiez pas aux endpoints REST d'enregistrement publics tant qu'ils ne sont pas livrés. Les clés YAML sous `recording:` configurent le stockage du pipeline.
...
- See the [Webhooks API](/fr/docs/api/api-reference) for endpoint details.
...
- [Backend Code Structure](/fr/docs/backend/structure) - directory map and coding standards
- [API Handlers](/fr/docs/backend/api-handlers) - routing and request lifecycle
- [Database and Models](/fr/docs/backend/database) - GORM models and repository pattern
- [Authentication Flow](/fr/docs/backend/authentication) - JWT, OAuth, and passkey internals
