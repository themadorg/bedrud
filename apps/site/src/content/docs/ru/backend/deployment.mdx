...
Prints the installed version and the version it is moving to before stopping anything, then replaces the binary, runs versioned install migrations and DB AutoMigrate, refreshes init units, and restarts services. See [Updating](/ru/docs/getting-started/installation#updating).
