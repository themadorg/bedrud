...
3. **Otomatik enjeksiyon** – `authFetch` her API çağrısını sarmalar ve `Authorization: Bearer` başlığını ekler (durum değiştiren istekler için zorunlu; sunucu yalnızca çerezli mutasyonları reddeder)
...
- **Chat** - Oda içi metin sohbeti (`ChatPanel`: tepkiler, anketler, ekler)
- **Spotlight view** - Focus a single participant
- **Stage mode** - Sunucu odaklı düzen ve sahne ekran paylaşımı katmanı
- **Whiteboard** - Ortak tahta (Yjs over LiveKit data channels); Deneysel ayarlar altında kapılı olabilir
- **YouTube watch** - Senkron izleme katmanı; isteğe bağlı Deneysel anahtar
- **Push-to-talk** - Ses tercihlerinde yapılandırılan basılı tutma tuşu ile iletim

Son kullanıcı kılavuzları: [Bedrud'u Kullanma](/tr/docs/using/joining-a-meeting) (katılmadan başlayın; sahne, beyaz tahta, sohbet ve kontroller).
