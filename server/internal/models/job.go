...
type Job struct {
	...
	// DedupeKey collapses repeat requests for the same operation while a job for
	...
	// only, so a finished job never blocks a later one. That composite index is
	// the only thing reading this column, so it carries no index tag of its own.
