...
3. **自动注入** – `authFetch` 包装每次 API 调用并添加 `Authorization: Bearer` 头（状态变更请求必需；服务器拒绝仅 Cookie 的变更）
...
- **Chat** - 房间内文字聊天（`ChatPanel`：回应、投票、附件）
- **Spotlight view** - Focus a single participant
- **Stage mode** - 演示者聚焦布局与舞台屏幕共享叠加层
- **Whiteboard** - 协作画板（Yjs over LiveKit data channels）；可在实验性设置下门控
- **YouTube watch** - 同步观看叠加层；可选实验性开关
- **Push-to-talk** - 在音频偏好中配置的按住键发送

终端用户教程：[使用 Bedrud](/zh/docs/using/joining-a-meeting)（从加入开始，再到舞台、白板、聊天与控制）。
