...
description: Diseño y componentes del servidor backend
order: 7
---
...
El middleware extrae el JWT del encabezado `Authorization: Bearer <token>` (reserva de cookie en lecturas), lo valida y adjunta el contexto de usuario a la solicitud. Las rutas protegidas usan `RequireAccess` para roles.

- Los **tokens con propósito** (`email_verify`, `password_reset`) comparten la clave de firma pero **no pueden** usarse como tokens de acceso en rutas `Protected`.
- Las **mutaciones** (POST/PUT/PATCH/DELETE autenticadas) exigen un encabezado Bearer vía `RequireBearerForMutations` — los POST solo con cookie cross-site se rechazan (defensa CSRF).
- La **revocación de tokens de acceso** (cierre de sesión / cambio de contraseña) se persiste en la base de datos y sobrevive reinicios y despliegues multiinstancia.
- Las listas de **identidad de presencia** de sala requieren autenticación; las salas públicas pueden exponer `?countOnly=1` (solo recuento, sin identidades).
...
### Grabaciones y LiveKit Egress

Los handlers de cola **`process_recording`** y **`recording_delete`** están registrados en el worker. Los webhooks LiveKit `egress_ended` encolan `process_recording` (descarga → almacenamiento → marcar completado → webhook opcional `recording.completed`).

🚧 Las **rutas HTTP de grabación** (start/stop/list en la API) siguen como `// TODO oncoming feature` en la tabla de rutas de producción — no confíe en endpoints REST públicos de grabación hasta que existan. Las claves YAML bajo `recording:` configuran el almacenamiento del pipeline.
...
- See the [Webhooks API](/es/docs/api/api-reference) for endpoint details.
...
- [Backend Code Structure](/es/docs/backend/structure) - directory map and coding standards
- [API Handlers](/es/docs/backend/api-handlers) - routing and request lifecycle
- [Database and Models](/es/docs/backend/database) - GORM models and repository pattern
- [Authentication Flow](/es/docs/backend/authentication) - JWT, OAuth, and passkey internals
