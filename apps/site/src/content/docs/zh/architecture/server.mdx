...
- **用途令牌**（`email_verify`、`password_reset`）与访问令牌共用签名密钥，但**不能**在 `Protected` 路由上当作访问令牌。
- **变更请求**（已认证的 POST/PUT/PATCH/DELETE）须通过 `RequireBearerForMutations` 携带 Bearer；仅 Cookie 的跨站 POST 会被拒绝（CSRF 防护）。
- **访问令牌吊销**（登出 / 改密）持久化到数据库，可跨进程重启与多实例部署生效。
- **房间在线身份列表**需要身份验证；公开房间可提供 `?countOnly=1`（仅人数，无身份）。
...
- See the [Webhooks API](/zh/docs/api/api-reference) for endpoint details.
...
- [Backend Code Structure](/zh/docs/backend/structure) - directory map and coding standards
- [API Handlers](/zh/docs/backend/api-handlers) - routing and request lifecycle
- [Database and Models](/zh/docs/backend/database) - GORM models and repository pattern
- [Authentication Flow](/zh/docs/backend/authentication) - JWT, OAuth, and passkey internals
