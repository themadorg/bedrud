...
See [WebRTC Connectivity](/zh/docs/architecture/webrtc-connectivity) for the full connectivity stack.
...
In embedded mode (default), when server TLS is enabled, TURN/TLS config is automatically generated from the server's TLS certificate — no manual configuration required. The TURN `domain` is auto-set from `server.host` in the Bedrud config, and `udp_port: 3478` is always configured alongside `tls_port: 5349`. For custom TURN settings, provide a LiveKit YAML via `livekit.configPath` or the `LIVEKIT_CONFIG_PATH` environment variable.
...
### `use_external_ip` / `node_ip` Interaction
...
In embedded mode, the node IP resolution follows this priority:
1. Explicit `livekit.nodeIP` in Bedrud config (sets `node_ip` and `use_external_ip: false`)
2. `server.host` in Bedrud config (if it's a valid non-loopback IP)
3. Outbound IP auto-detection (UDP dial) — only when neither of the above is set does `use_external_ip: true` apply

When `use_external_ip: true`, LiveKit uses STUN to discover the public IP. This requires outbound internet access and fails on air-gapped networks. When `node_ip` is explicitly set, `use_external_ip` is `false` and LiveKit uses the provided IP directly — required for LAN deployments without internet.
...
- [WebRTC Connectivity](/zh/docs/architecture/webrtc-connectivity) - full STUN/ICE/TURN/SFU connectivity stack
- [LiveKit Integration](/zh/docs/backend/livekit) - how Bedrud embeds LiveKit
- [Configuration Reference](/zh/docs/getting-started/configuration) - all config options
- [Internal TLS](/zh/docs/guides/internal-tls) - TLS for isolated networks
- [Deployment Guide](/zh/docs/guides/deployment) - production deployment steps
