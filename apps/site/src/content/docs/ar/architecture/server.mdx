---
title: بنية الخادم
description: تصميم خادم الخلفية ومكوناته
order: 7
---
...
تستخرج البرمجية الوسيطة JWT من الترويسة `Authorization: Bearer <token>` (مع احتياطي الكوكي للقراءة)، وتتحقق منه، وتُرفق سياق المستخدم بالطلب. تستخدم المسارات المحمية `RequireAccess` للأدوار.

- **رموز الغرض** (`email_verify`، `password_reset`) تشارك مفتاح التوقيع لكن **لا يجوز** استخدامها كرموز وصول على مسارات `Protected`.
- **طلبات التغيير** (POST/PUT/PATCH/DELETE المصادَق عليها) تتطلب ترويسة Bearer عبر `RequireBearerForMutations` — تُرفض طلبات POST عبر المواقع بالكوكي فقط (حماية CSRF).
- **إبطال رمز الوصول** (تسجيل الخروج / تغيير كلمة المرور) يُخزَّن في قاعدة البيانات ويصمد أمام إعادة التشغيل والنشر متعدد النسخ.
- قوائم **هويات الحضور** في الغرفة تتطلب مصادقة؛ قد تعرض الغرف العامة `?countOnly=1` (العدد فقط بلا هويات).
...
### التسجيلات و LiveKit Egress

مُعالِجات الطابور **`process_recording`** و **`recording_delete`** مسجَّلة على العامل. تُدرِج خطافات LiveKit `egress_ended` مهمة `process_recording` (تنزيل → تخزين → تعليم كمكتمل → خطاف اختياري `recording.completed`).

🚧 **مسارات HTTP للتسجيل** (start/stop/list عبر API) ما تزال `// TODO oncoming feature` في جدول المسارات الإنتاجي — لا تعتمد على نقاط REST عامة للتسجيل حتى تُتاح. مفاتيح YAML تحت `recording:` تضبط تخزين خط المعالجة.
...
- See the [Webhooks API](/ar/docs/api/api-reference) for endpoint details.
...
- [Backend Code Structure](/ar/docs/backend/structure) - directory map and coding standards
- [API Handlers](/ar/docs/backend/api-handlers) - routing and request lifecycle
- [Database and Models](/ar/docs/backend/database) - GORM models and repository pattern
- [Authentication Flow](/ar/docs/backend/authentication) - JWT, OAuth, and passkey internals
