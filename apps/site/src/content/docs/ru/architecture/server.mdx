...
description: Дизайн и компоненты бэкенд-сервера
order: 7
---
...
Middleware извлекает JWT из заголовка `Authorization: Bearer <token>` (fallback на cookie при чтении), проверяет его и добавляет контекст пользователя к запросу. Защищённые маршруты используют `RequireAccess` для ролей.

- **Токены с purpose** (`email_verify`, `password_reset`) делят ключ подписи, но **нельзя** использовать их как access-токены на маршрутах `Protected`.
- **Мутации** (аутентифицированные POST/PUT/PATCH/DELETE) требуют Bearer через `RequireBearerForMutations` — cookie-only cross-site POST отклоняются (защита CSRF).
- **Отзыв access-токена** (выход / смена пароля) сохраняется в БД и переживает перезапуски и multi-instance.
- Списки **личностей присутствия** в комнате требуют аутентификации; публичные комнаты могут отдавать `?countOnly=1` (только счётчик, без личностей).
...
### Записи и LiveKit Egress

Обработчики очереди **`process_recording`** и **`recording_delete`** зарегистрированы на worker. Webhook LiveKit `egress_ended` ставит в очередь `process_recording` (скачивание → хранение → completed → опциональный webhook `recording.completed`).

🚧 **HTTP-маршруты записи** (start/stop/list API) по-прежнему `// TODO oncoming feature` в production route table — не полагайтесь на публичные REST записи, пока они не появятся. Ключи YAML под `recording:` настраивают хранилище конвейера.
...
- See the [Webhooks API](/ru/docs/api/api-reference) for endpoint details.
...
- [Backend Code Structure](/ru/docs/backend/structure) - directory map and coding standards
- [API Handlers](/ru/docs/backend/api-handlers) - routing and request lifecycle
- [Database and Models](/ru/docs/backend/database) - GORM models and repository pattern
- [Authentication Flow](/ru/docs/backend/authentication) - JWT, OAuth, and passkey internals
