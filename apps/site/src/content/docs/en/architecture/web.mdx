...
3. **Automatic Injection** - `authFetch` wraps every API call, adding the `Authorization: Bearer` header (required for state-changing requests; cookie-only mutations are rejected by the server)
...
End-user how-tos: [Using Bedrud](/en/docs/using/joining-a-meeting) (start at joining, then stage, whiteboard, chat, and controls).
