...
| 区域 | 行为 |
|------|------|
| Auth | 登录 / 注册（以及支持处的通行密钥） |
| Dashboard | 房间列表；加入后的**最近房间** |
| Deep links | 粘贴 / 打开的房间 URL 解析到正确房间 |
| Meeting | 加入、静音、离开；可用的控制栏 |
| Stage / share | 设备允许时的舞台叠加层与屏幕共享路径 |
| Telecom | 自管理通话连接；断开干净结束会议 |
| RTL | `fa` / `ar` 语言与 Shabnam — 外壳不应裁切 |

共享产品概念（web 上的舞台、聊天、白板）见 [使用 Bedrud](/zh/docs/using/joining-a-meeting)。与 web 的功能对等可能因版本而异。
