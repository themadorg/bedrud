...
In embedded mode (default), when server TLS is enabled, TURN/TLS config is automatically generated from the server's TLS certificate — no manual configuration required. For custom TURN settings, provide a LiveKit YAML via `livekit.configPath` or the `LIVEKIT_CONFIG_PATH` environment variable.
