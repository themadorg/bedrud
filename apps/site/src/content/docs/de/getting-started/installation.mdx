...
Download the latest release and re-run install. Without `--fresh`, the installer refreshes the binary and services and **does not overwrite** existing `config.yaml` / `livekit.yaml`. For a binary-only upgrade prefer `bedrud update`. A clean wipe uses `--fresh`:
