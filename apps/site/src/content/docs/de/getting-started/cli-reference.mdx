...
| `--fresh`            | `false`                         | Wipe existing install (config, data, certs) then reinstall. Without this, existing `config.yaml` / `livekit.yaml` are left unchanged |
...
5. Writes `/etc/bedrud/config.yaml` only if it does not already exist (re-run without `--fresh` preserves it)
6. Writes `/etc/bedrud/livekit.yaml` only if it does not already exist
...
**Related docs:** [Installation Guide](/de/docs/getting-started/installation) • [Deployment Guide](/de/docs/guides/deployment) • [Backend Deployment Logic](/de/docs/backend/deployment)
...
| `--check` | `false` | Report the target version and exit without changing anything |
| `--self` | `false` | Install from the running executable |
...
| `--skip-checksum` | `false` | Skip SHA256 verification (local/trusted sources only; never with `latest`) |
| `--json` | `false` | Machine-readable JSON output |

**What it does:**
1. Verifies an existing install (`/etc/bedrud/config.yaml` or `--config`)
2. Resolves the source and reports the version it installs — before anything is stopped or replaced
3. Stops `bedrud` and `livekit` services
4. Replaces the installed binary with the resolved source (skips overwrite for package-managed `/usr/bin/bedrud` when the running binary is already that path; otherwise installs to `/usr/local/bin`)
5. Runs versioned install-state migrations when upgrading across releases that need them
6. Runs database AutoMigrate (same as `bedrud db migrate` / server startup)
7. Rewrites init unit files so `ExecStart` matches the binary path and LiveKit topology
8. Enables and restarts services
9. Records the installed version in `/var/lib/bedrud/version` (read back from the binary when the source carries no release tag)

**Typical workflows:**

```bash
# See what an update would install — changes nothing
sudo bedrud update --check

# Download and install the latest release
sudo bedrud update latest

# Download the binary first, then apply it with its absolute path.
# sudo on the installer keeps it in /usr/local/bin; without it the binary
# lands in ~/.local/bin and the line below would re-apply the old one.
...
**Implementation:** `server/internal/install/update.go`, `update_check.go`, `update_version.go`, `version.go`, `services.go`

**Related docs:** [Installation Guide - Updating](/de/docs/getting-started/installation#updating) • [Appliance Mode](/de/docs/guides/appliance)
...
**Related docs:** [Installation Guide - Uninstalling](/de/docs/getting-started/installation#uninstallation)
...
- [Installation Guide](/de/docs/getting-started/installation) - Detailed `bedrud install` walkthrough, TLS topologies, post-install steps
- [Configuration Reference](/de/docs/getting-started/configuration) - Server and LiveKit config file structure
- [Backend Deployment Logic](/de/docs/backend/deployment) - How `install` and `uninstall` work internally
- [Deployment Guide](/de/docs/guides/deployment) - Full deployment options including init system services
