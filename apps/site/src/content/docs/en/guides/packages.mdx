...
Download the `.deb` package directly from the [latest GitHub release](https://github.com/themadorg/bedrud/releases/latest):
...
<Tabs>
  ...
  <TabsContent value="x86-64">
    ```bash
    wget https://github.com/themadorg/bedrud/releases/latest/download/bedrud_amd64.deb
  ...
  <TabsContent value="arm64">
    ```bash
    wget https://github.com/themadorg/bedrud/releases/latest/download/bedrud_arm64.deb
...
Download the `.rpm` package from the [latest GitHub release](https://github.com/themadorg/bedrud/releases/latest):
...
<Tabs>
  ...
  <TabsContent value="x86-64">
    ```bash
    sudo dnf install https://github.com/themadorg/bedrud/releases/latest/download/bedrud-*.x86_64.rpm
    ```
  </TabsContent>

  <TabsContent value="aarch64">
    ```bash
    sudo dnf install https://github.com/themadorg/bedrud/releases/latest/download/bedrud-*.aarch64.rpm
    ```
  </TabsContent>
</Tabs>
...
Download the compressed tarball and extract it anywhere on your `$PATH`. Static binary — works on all Linux distributions (Debian, Ubuntu, Fedora, Alpine, etc).
...
<Tabs>
  ...
  <TabsContent value="linux-x86-64">
    ```bash
    curl -L https://github.com/themadorg/bedrud/releases/latest/download/bedrud_linux_amd64.tar.xz \
      | tar xJ
  ...
  <TabsContent value="linux-arm64">
    ```bash
    curl -L https://github.com/themadorg/bedrud/releases/latest/download/bedrud_linux_arm64.tar.xz \
      | tar xJ
    sudo mv bedrud /usr/local/bin/
    ```
  </TabsContent>
...
Download the Alpine package from the [latest release](https://github.com/themadorg/bedrud/releases/latest):
...
# ARM64 (aarch64)
wget https://github.com/themadorg/bedrud/releases/latest/download/bedrud_aarch64.apk
apk add --allow-untrusted bedrud_aarch64.apk
```

### Docker

Three image variants are available on GitHub Container Registry:

| Tag | Base | Size |
|-----|------|------|
| `:latest` | Debian Bookworm | ~50 MB |
| `:latest-alpine` | Alpine 3.21 | ~30 MB |
| `:latest-distroless` | Distroless | ~25 MB |
...
After a package or binary upgrade, apply migrations and refresh services:

```bash
sudo apt upgrade bedrud          # or: dnf upgrade bedrud

# the package already replaced the binary — apply migrations and restart:
sudo bedrud update --skip-binary

# for a binary install, fetch and apply the release itself:
sudo bedrud update latest
```

`update` and `upgrade` are aliases. Details: [Updating](/en/docs/getting-started/installation#updating).
...
<Tabs>
  ...
  <TabsContent value="flatpak">
    ...
    ```bash
    flatpak install https://github.com/themadorg/bedrud/releases/latest/download/bedrud-desktop-linux-x86_64.flatpak
    flatpak runorgbedrud.Desktop
    ```
  </TabsContent>
...
<Tabs>
  ...
  <TabsContent value="arm64">
    ```bash
    curl -L https://github.com/themadorg/bedrud/releases/latest/download/bedrud-desktop-macos-arm64.tar.gz \
      | tar xz
  ...
  <TabsContent value="x86-64">
    ```bash
    curl -L https://github.com/themadorg/bedrud/releases/latest/download/bedrud-desktop-macos-x86_64.tar.gz \
      | tar xz
    xattr -d com.apple.quarantine bedrud-desktop
    ./bedrud-desktop
    ```
...
<Tabs>
  ...
  <TabsContent value="nsis">
    Download and run the installer from the [latest release](https://github.com/themadorg/bedrud/releases/latest):
...
| Alpine Linux | `apk` / `tar.xz` (static) | - |
| Docker | `ghcr.io/themadorg/bedrud` (`:latest` / `:latest-alpine`) | - |
...
| `HOMEBREW_TAP_TOKEN` | Homebrew tap | GitHub PAT with write access to `themadorg/homebrew-bedrud` |
