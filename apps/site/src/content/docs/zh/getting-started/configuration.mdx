...
省略 `rateLimit` 部分以保持默认值。设置 `authMaxRequests: 0` 或 `guestMaxRequests: 0` 以禁用。
...
**路径：** `server/livekit.yaml`（开发环境）或 `/etc/bedrud/livekit.yaml`（生产环境）

<Tabs>
  <TabsList>
    <TabsTrigger value="production">生产 / 外部 (标准)</TabsTrigger>
    <TabsTrigger value="development">本地开发 (回环)</TabsTrigger>
  </TabsList>
...
| `AUTH_PASSKEY_CHALLENGE_TTL` | `auth.passkeyChallengeTTL` | Passkey 挑战过期时间（分钟） |
...
| `RATELIMIT_AUTH_MAX` | `rateLimit.authMaxRequests` | 每个窗口的最大认证请求数 |
| `RATELIMIT_AUTH_WINDOW` | `rateLimit.authWindowSecs` | 认证速率限制窗口（秒） |
| `RATELIMIT_GUEST_MAX` | `rateLimit.guestMaxRequests` | 每个窗口的最大访客加入请求数 |
| `RATELIMIT_GUEST_WINDOW` | `rateLimit.guestWindowSecs` | 访客速率限制窗口（秒） |
