...
3. **Injection automatique** – `authFetch` enveloppe chaque appel API et ajoute l'en-tête `Authorization: Bearer` (requis pour les requêtes mutantes ; les mutations cookie seules sont rejetées par le serveur)
...
- **Chat** - Chat texte en salle via `ChatPanel` (réactions, sondages, pièces jointes)
- **Spotlight view** - Focus a single participant
- **Mode scène** - Disposition centrée sur le présentateur et overlay de partage d'écran de scène
- **Tableau blanc** - Tableau collaboratif (Yjs sur data channels LiveKit) ; peut être derrière les paramètres expérimentaux
- **YouTube Watch** - Overlay de visionnage synchronisé ; bascule expérimentale optionnelle
- **Push-to-talk** - Émission à la touche maintenue, configurée dans les préférences audio

How-tos utilisateur final : [Utiliser Bedrud](/fr/docs/using/joining-a-meeting) (commencer par rejoindre, puis scène, tableau blanc, chat et contrôles).
...
- [Web Design System](https://github.com/themadorg/bedrud/tree/master/apps/web/AGENTS.md) - UI component patterns, color tokens, and styling rules
