...
3. **Automatische Injektion** – `authFetch` umschließt jeden API-Aufruf und setzt den Header `Authorization: Bearer` (für zustandsändernde Anfragen erforderlich; reine Cookie-Mutationen lehnt der Server ab)
...
- **Chat** - Textchat im Raum über `ChatPanel` (Reaktionen, Umfragen, Anhänge)
- **Spotlight view** - Focus a single participant
- **Stage-Modus** - Präsentations-Layout und Stage-Bildschirmfreigabe-Overlay
- **Whiteboard** - Kollaboratives Board (Yjs über LiveKit-Datenkanäle); ggf. unter Experimentellen Einstellungen freigeschaltet
- **YouTube Watch** - Synchronisiertes Watch-Overlay; optionaler experimenteller Schalter
- **Push-to-Talk** - Halten-zum-Senden, konfiguriert in den Audio-Präferenzen

Endnutzer-How-tos: [Bedrud nutzen](/de/docs/using/joining-a-meeting) (Start bei Beitreten, dann Stage, Whiteboard, Chat und Steuerung).
