...
/** Vendor Excalidraw UI only downloads when whiteboard is opened (client-only). */
const MeetingSharedWhiteboard = lazy(() =>
  import('@/components/meeting/whiteboard/MeetingSharedWhiteboard').then((m) => ({
    default: m.MeetingSharedWhiteboard,
  })),
)
...
export function WhiteboardOverlay() {
  ...
  // Never SSR-render Excalidraw — hooks need a real client React dispatcher.
