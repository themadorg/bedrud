...
### Superficie de producto (lo que obtienen los usuarios)

| Área | Comportamiento |
|------|----------------|
| Auth | Login / registro (y passkeys donde se admitan) |
| Panel | Lista de salas; **salas recientes** tras unirse |
| Deep links | Las URL de sala pegadas / abiertas resuelven la sala correcta |
| Reunión | Unirse, silenciar, salir; chrome de controles usable |
| Escenario / compartir | Superposición de escenario y camino de compartir pantalla cuando el dispositivo lo permite |
| Telecom | Conexión de llamada autogestionada; desconectar termina la reunión de forma limpia |
| RTL | Locales `fa` / `ar` con Shabnam — el chrome no debería recortarse |

Los conceptos de producto compartidos (escenario, chat, pizarra en web) están documentados en [Usar Bedrud](/es/docs/using/joining-a-meeting). La paridad de funciones con web puede diferir por release.
