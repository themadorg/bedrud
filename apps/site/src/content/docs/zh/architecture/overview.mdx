...
flowchart TB
    ...
    subgraph Server ["Bedrud Server"]
        ...
        Queue["Job Queue<br/>(async deletion, uploads)"]
...
- **Async job queue** - background worker processes room deletion, user deletion, suspensions, and chat uploads without blocking API responses
...
See [Server Architecture](/zh/docs/architecture/server) for details.
...
See [Web Frontend](/zh/docs/architecture/web) for details.
...
See [Android App](/zh/docs/architecture/android) for details.
...
See [iOS App](/zh/docs/architecture/ios) for details.
...
See [Desktop App](/zh/docs/architecture/desktop) for details.
...
See [Bot Agents](/zh/docs/architecture/agents) for details.
...
In production, Bedrud runs as two init services (systemd, OpenRC, or SysV init):
...
See [Deployment Guide](/zh/docs/guides/deployment) for setup instructions.
...
- [TURN Server Guide](/zh/docs/architecture/turn-server) - TURN relay architecture and configuration
