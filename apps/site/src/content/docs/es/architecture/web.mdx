...
3. **Inyección automática** – `authFetch` envuelve cada llamada API y añade el encabezado `Authorization: Bearer` (obligatorio para peticiones que cambian estado; el servidor rechaza mutaciones solo con cookie)
...
- **Chat** - Chat de texto en sala vía `ChatPanel` (reacciones, encuestas, adjuntos)
- **Spotlight view** - Focus a single participant
- **Modo escenario** - Diseño centrado en el presentador y superposición de compartir pantalla del escenario
- **Pizarra** - Tablero colaborativo (Yjs sobre data channels de LiveKit); puede estar bajo Ajustes experimentales
- **YouTube Watch** - Superposición de visionado sincronizado; conmutador experimental opcional
- **Push-to-talk** - Transmitir al mantener tecla, configurado en preferencias de audio

How-tos de usuario final: [Usar Bedrud](/es/docs/using/joining-a-meeting) (empieza por unirse, luego escenario, pizarra, chat y controles).
